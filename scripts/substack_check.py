...
EMAIL_ANYWHERE_RE = re.compile(r"[^@\s,;]+@[^@\s,;]+\.[^@\s,;]+")


def _sanitize_value(value):
    """Redact any embedded email and truncate; evidence strings reach model-visible JSON."""
    value = EMAIL_ANYWHERE_RE.sub(lambda m: redact_email(m.group(0)), value)
    return value[:40]
    ...
    for name in components["email_lists"]:
        ...
        if parsed != physical:
            parse_warnings.append(
                ...
                "quotes or quoted multi-line fields; some subscribers may be "
                "missing from the cleaned CSV" % (name, parsed, physical)
            )
