...
class TestErrorPaths(unittest.TestCase):
    ...
    def test_cli_audit_failed_exit_3(self):
        path = build_full_export(os.path.join(self.tmp.name, "full.zip"))
        _corrupt_first_deflated_member(path)
        proc = subprocess.run(
            [sys.executable, SCRIPT, path,
        ...
        payload = json_mod.loads(proc.stdout)
        self.assertEqual(payload["error"], "audit_failed")

    def test_embedded_email_in_evidence_is_sanitized(self):
        self.assertNotIn(
            "hidden.person@private.example",
            sc._sanitize_value("plan for hidden.person@private.example"),
        )
